defmodule Register do
  ...
  def reset(register, zero) do
    %{register | regs: [zero, 0, 0, 0, 0, 0], ip: 0, halted?: false, zero: zero}
  end

  def step(register = %{halted?: true}), do: register
...
|> Stream.map(&if(&1.ip in [28], do: Enum.at(&1.regs, 5), else: nil))
|> Stream.filter(& &1)
|> Stream.take(10)
