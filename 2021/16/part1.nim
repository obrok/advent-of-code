...
proc parsePacket(input: seq[int], pos: int): ParseResult[Packet] =
  ...
  case type_id:
    of 4:
      ...
      return (res: Packet(version: version, value: temp.res), pos: temp.pos)
    else:
      case input[pos + 6]:
        of 1:
          let nchildren = number(input[pos + 7 .. pos + 17])
          let children = parseChildren(input, pos + 18, nchildren)
          return (res: Packet(version: version, children: children.res), pos: children.pos)
        else:
...
proc print(packet: Packet): string =
  &"({packet.version}, {packet.value}, [{packet.children.mapIt(it.print).join(\",\")}])"
...
echo parsePacket(input, 0).res.totalVersion
